...
- inverse is \e not supported. Use one of the @link gDecomps matrix decompositions @endlink instead

See individual member function documentation for examples of usage.

\par Statically-sized matrices

The library provides classes for statically and dynamically sized matrices. As with @link Vector Vectors@endlink, statically sized matrices are more efficient, since their size is determined at compile-time, not run-time. 
...
Matrix<3> M;
@endcode
which just is a synonym for <code>Matrix<3,3></code>. Matrices can also be constructed 
...
@endcode
where \a num_rows and \a num_cols are integers which will be evaluated at run time. 
Dynamically-sized matrices can be constructed from pointers in a similar manner to static sized matrices:
@code
...
The library supports both row major (the default - but you can change this if you prefer) and 
column major layout ordering. Row major implies that the matrix is laid out in memory in 
raster scan order:
\f[\begin{matrix}\text{Row major} & \text {Column major}\\\begin{bmatrix}1&2&3\\4&5&6\\7&8&9\end{bmatrix} & \begin{bmatrix}1&4&7\\2&5&8\\3&6&9\end{bmatrix} \end{matrix}\f]
...
public:
	...
	/// row major (along the rows first i.e. raster scan order), unless <code>ColMajor</code> is specified
	/// in the template arguments (see detailed description). No errors are generated if the array is the 
	/// wrong length.
	/// @code
	/// double d[4] = {1, 2, 3, 4};
	...
	/// Construct a (fixed-size) matrix from a 2D array of doubles. No errors are generated if the array is the 
	/// wrong length.
	/// @code
	...
	//@{
	/// The transpose of the matrix. This is a very fast operation--it simply reinterprets a row-major
	/// matrix as column-major or vice-versa. This can be used as an l-value.
	/// @code
	/// double d[2][3] = {{1, 2, 3}, {4, 5, 6}};
	/// Matrix<2,3> m(d);
	/// Zero(m[0]);   // set the first row to zero
	...
	/// The transpose of the matrix. This is a very fast operation--it simply reinterprets a row-major
	/// matrix as column-major or vice-versa. The result can be used as an l-value. 
	/// @code
	/// double d[2][3] = {{1, 2, 3}, {4, 5, 6}};
	/// Matrix<2,3> m(d);
	/// Vector<2> v = 8,9;
	...
	///               //         [9 5 6]
	/// @endcode
	/// <b>This means that the semantics of <code>M=M.T()</code> are broken</b>. In general, it is not 
	/// necessary to say <code>M=M.T()</code>, since you can use M.T() for free whenever you need the transpose,
	/// but if you do need to, you have to use the Tranpose() function defined in <code>helpers.h</code>.
	Matrix<Cols, Rows>& T();
	
	/// Extract a sub-matrix. The matrix extracted will be begin at element (Rstart, Cstart) and will 
	...
	/// Extract a sub-matrix. The matrix extracted will be begin at element (Rstart, Cstart) and will 
	/// contain the next Rsize by Csize elements. This can be used as either an r-value or an l-value.
	/// @code
	/// double d[2][3] = {{1, 2, 3}, {4, 5, 6}};
	/// Matrix<2,3> m(d);
...
/// Write the matrix to a stream. The elements are space-separated with rows being separated by
/// new lines. The numbers are printed with a minimum field width of 12 characters.
/// @relates Matrix
template <int Rows, Cols>
std::ostream& operator<< (std::ostream& os, const Matrix<Rows, Cols>& v);

/// Read a matrix from a stream. The numbers can be comma or white-space separated, and are assumed
/// to be in row-major layout (i.e. scanline order) unless the matrix is defined as <code>ColMajor</code>
/// @relates Matrix
template <int Rows, Cols>
