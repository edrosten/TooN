...
namespace TooN{
	namespace Internal{
		...
		///@ingroup gInternal
		template<class Functor, class  Precision, int Size, class Base> struct Difference
		{
			...
			Difference(const Vector<Size, Precision, Base>& v_, const Functor& f_)
			:v(v_),x(v),f(f_),i(0)
			{}
	...
	*/
	template<class F, int S, class P, class B> Vector<S, P> numerical_gradient(const F& f, const Vector<S, P, B>& x)
	{
		...
		{
			d.i = i;
			grad[i] = extrapolate_to_zero<Difference<F, P, S, B>, P>(d).first;
		}
	...
	template<class F, int S, class P, class B> Matrix<S,2,P> numerical_gradient_with_errors(const F& f, const Vector<S, P, B>& x)
	{
		...
		{
			d.i = i;
			pair<double, double> r= extrapolate_to_zero<Difference<F, P, S, B>, P>(d);
			g[i][0] = r.first;
			g[i][1] = r.second;
		}
